...
local json = (loadfile "/home/یوزر سرور/inline/data/JSON.lua")()
local token = 'توکن رباتی که از بات فادر گرفتین' --token
local url = 'https://api.telegram.org/bot' .. token
local offset = 0
local redis = require('redis')
local redis = redis.connect('127.0.0.1', 6379)
local SUDO = 310083510
        ...
        for i=1, #updates.result do
          ...
          if msg.inline_query then
            local q = msg.inline_query
						if q.from.id == 325010791 or q.from.id == 310083510 then
          ...
          if msg.callback_query then
            local q = msg.callback_query
             ...
             if q.data:match('_') and not (q.data:match('next_page') or q.data:match('left_page')) then
                Canswer(q.id,">برای مشاهده راهنمای بیشتر این بخش عبارت\n/help\nرا ارسال کنید\n>تیم پشتیبانی:[@bom_bang_team]\n>کانال پشتیبانی:[@bom_bang_team]\n> فروش :[@sudo_monsters]",true)
